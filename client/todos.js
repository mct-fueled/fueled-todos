...
UserBase = new Meteor.Collection('userbase');
...
// When user is signed in, assign the profile ID
Session.setDefault('user_id', Meteor.userId());
Session.setDefault('all_users', null);
/*
var getUsername = Meteor.users.find({
	_id: Meteor.userId()
}).fetch();
Session.setDefault('username', getUsername[0].profile.name);
*/

Session.setDefault('user_page_id', null);
...
Template.todosHeader.fullName = function() {
	return Meteor.user().profile.name;
},
Template.userList.users = function() {
	return Meteor.users.find();
},
Template.user.fullName = function() {
	return Meteor.user().profile.name;
};

Template.user.events({
	'mousedown .user': function(evt) {
		Router.setPerson(this._id);
	},
...
var TodosRouter = Backbone.Router.extend({
	routes: {
		"project/:list_id": "projects",
		"people/:user_id": "people"
	},
	projects: function (list_id) {
		var oldList = Session.get("list_id");
		if (oldList !== list_id) {
			Session.set("list_id", list_id);
			Session.set('user_page_id', null);
			Session.set("tag_filter", null);
		}
	},
	people: function(user_id) {
		var oldPerson = Session.get('user_page_id');
		if(oldPerson !== user_id) {
			Session.set('list_id', null);
			Session.set('user_page_id', user_id);
			Session.set('tag_filter', null);
		}
	},
	setPerson: function(user_id) {
		this.navigate('people/'+user_id, true);
	},
	setProject: function (list_id) {
		this.navigate('projects/'+list_id, true);
	}
});
